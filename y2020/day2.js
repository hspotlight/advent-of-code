...
function getAnswer(input) {
    ...
    const [x,min, max,character, text] = pattern.exec(input)

    const count = countChar(character, text)
...
function getAnswer2(input) {
    const pattern = /^(\d+)-(\d+) (\w): (\w+)$/
    const [x, pos1,pos2,character, text] = pattern.exec(input)

    const a = text[pos1-1] === character;
    const b = text[pos2-1] === character;
    if (a ^ b) {
        return true;
    }
    return false;
...
function countChar(character, text) {
  const pattern = new RegExp(character, 'g');
  return (text.match(pattern) || []).length;
}

const inputs = ["1-14 b: bbbbbbbbbbbbbbbbbbb",
